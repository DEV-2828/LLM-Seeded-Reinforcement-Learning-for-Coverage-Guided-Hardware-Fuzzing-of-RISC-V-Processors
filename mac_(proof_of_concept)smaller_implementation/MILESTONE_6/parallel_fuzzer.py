...
def worker_loop(worker_id, num_episodes, max_steps, experience_queue, weight_dict):
    ...
    if verilator_env_path not in sys.path:
        sys.path.insert(0, verilator_env_path)
    parent_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    if parent_path not in sys.path:
        sys.path.insert(0, parent_path)

    from differential_fuzzer import DifferentialFuzzerEnv
...
def main():
    # Use spawn method for safety with ctypes and CUDA (required on macOS)
    ...
    while any(p.is_alive() for p in workers) or not experience_queue.empty():
        while not experience_queue.empty():
            ...
            if steps_processed % 1000 == 0:
                ...
                try:
                    qsize = experience_queue.qsize()
                except NotImplementedError:
                    qsize = "N/A"  # qsize() not supported on macOS
                print(f"[Main] Processed {steps_processed} transitions. Queue size: {qsize}. Time: {elapsed:.1f}s")
